...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    const playerContract = await (
      await ethers.getContractFactory("FaultyContract", player2)
    ).deploy(token.address, lendingPool.address, uniswapExchange.address);

    await token
      .connect(player)
      .transfer(playerContract.address, PLAYER_INITIAL_TOKEN_BALANCE);
    ...
    await token
      .connect(player2)
      .transfer(player.address, POOL_INITIAL_TOKEN_BALANCE);
  });
